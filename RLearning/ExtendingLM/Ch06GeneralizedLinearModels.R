...
plot(resid(modp) ~ predict(modp, type="link"), 
     xlab=expression(hat(eta)),
 	 ylab="Deviance residuals")
# Note, this (eta.hat) is the first plot given for plot(model.glm):
par(mfrow=c(2, 2))
plot(modp)
...
# The response variable is count (Poisson-distributed), so the non-constant 
# variance is expected, and why we want to use a glm in the first place

# Examine relationship between predictors and response
...
# though, as it fails to account for the effects of the other variables.
...
plot(u ~ log(Area), gala, ylab="Partial Residual", xlab="log(Area)")
...
abline(0, coef(modpl)[2], col=2)
plot(residuals(modpl, type="partial")[, 2] ~ log(gala$Elevation)) 
abline(0, coef(modpl)[3], col=2)
plot(residuals(modpl, type="partial")[, 3] ~ log(gala$Nearest)) 
abline(0, coef(modpl)[4], col=2)
plot(residuals(modpl, type="partial")[, 4] ~ log(gala$Scruz + 0.1)) 
abline(0, coef(modpl)[5], col=2)
plot(residuals(modpl, type="partial")[, 5] ~ log(gala$Adjacent)) 
abline(0, coef(modpl)[6], col=2)

# Check assumption of validitiy of the particular link function: (plot z 
# against eta)
z <- predict(modpl) + (gala$Species - mu)/mu
par(mfrow=c(1, 1))
plot(z ~ predict(modpl), 
     xlab="Linear predictor (z)", 
	 ylab="Linearized response (eta)")
